      ...
      <b-col col lg="3">
        <b-card :title="yearSelectLocal" sub-title="Move cursor to change year">
          <div style="display: flex; align-items: center; justify-content:center;">
            <b-button pill variant="outline-secondary" style="padding: 6px 15px;" @click="minus">-</b-button>
            <b-form-input v-if="dataType == 'net_migration' || dataType=='migration_perc' " id="range-2" v-model="yearSelectLocal" @change.native="changeYearLocal" type="range" min="1962"
                          max="2018" step="5"
                          style="width: 200px;"></b-form-input>
            <b-form-input v-if="dataType != 'net_migration' && dataType!='migration_perc' " id="range-1" v-model="yearSelectLocal" @change.native="changeYearLocal" type="range" min="1960"
                          max="2018"
                          style="width: 200px;"></b-form-input>
            <b-button pill variant="outline-secondary" @click="plus">+</b-button>
          </div>
        </b-card>
      </b-col>
    </b-row>
